...
def leer_tarjetas_libros(sopa_pagina, url_pagina):
    ...
    for tarjeta in sopa_pagina.select("article.product_pod"):
        ...
        try:
            ...
            precio = None  # Si no se puede convertir
...
def leer_detalle_libro(url_detalle):
    ...
    upc = datos.get("UPC", "")
    try:
        precio_sin_imp = float(datos.get("Price (excl. tax)", "£0").replace("£", ""))
    except ValueError:
        precio_sin_imp = None
    try:
        precio_con_imp = float(datos.get("Price (incl. tax)", "£0").replace("£", ""))
    except ValueError:
        precio_con_imp = None
    try:
        impuesto = float(datos.get("Tax", "£0").replace("£", ""))
    except ValueError:
        impuesto = None
...
if __name__ == "__main__":
    ...
    # Mostrar un ejemplo de libro
    if libros:
        print("Ejemplo de un libro obtenido:")
        ejemplo = libros[0]
